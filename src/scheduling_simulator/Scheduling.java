...
public class Scheduling {
  ...
  private static int processnum = 7;
  private static int meanDev = 1000;
  private static int standardDev = 100;
  private static int runtime = 1000;
  private static Vector processVector = new Vector();
  private static Results result = new Results("null","null",0);
  private static String resultsFile = "result_data/Summary-Results";

  private static void Init(String file) {
    File f = new File(file);
    String line;
    String tmp;
    sProcess readProcess = new sProcess();
    double X = 0.0;
    ...
    try {   
      ...
      while ((line = in.readLine()) != null) {
        ...
        if (line.startsWith("process")) {
          StringTokenizer st = new StringTokenizer(line);
          st.nextToken();
          readProcess.ioblocking = Common.s2i(st.nextToken());
          readProcess.arrivalTime = Common.s2i(st.nextToken());
          X = Common.R1();
          while (X == -1.0) {
            X = Common.R1();
          }
          ...
          processVector.addElement(new sProcess(readProcess.cputime, readProcess.ioblocking, 0, 0, 0,
                  readProcess.arrivalTime));
        }
  ...
  public static void main(String[] args) {
    ...
    String config_path = "resources/scheduling.conf";
