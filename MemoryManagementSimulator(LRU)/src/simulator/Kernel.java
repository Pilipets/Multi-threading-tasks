...
public class Kernel extends Thread
{
                ...
                while ((line = in.readLine()) != null)
                {
                    if (line.startsWith("numpages"))
                    {
                        ...
                        while (st.hasMoreTokens())
                        {
                            ...
                            cache = new LRUCache((virtPageNum+1)/2);
                        }
                    }
                }
                in.close();
    ...
    public void step()
    {
        ...
        int phys_req_num = Virtual2Physical.pageNum( instruct.addr , virtPageNum , block );
        getPage( phys_req_num );
        ...
        if ( instruct.inst.startsWith( "READ" ) )
        {
            if ( cache.get(phys_req_num) == -1 )
            {
                ...
                PageFault.replacePage( memVector , cache, virtPageNum , phys_req_num , controlPanel );
                controlPanel.pageFaultValueLabel.setText( "YES" );
            }
            else
            {
                Page page = ( Page ) memVector.elementAt( phys_req_num );
                page.R = 1;
